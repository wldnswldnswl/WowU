...
//            Log.d("Bodypoint", "index[" + k + "] = " + " " + result[0] + " " + result[1] + " ");
       ...
       if(wait10sec==0){
            ...
            Log.i("downflag",down_flag+"");
            if(
                    down_flag==1&&
                    ((standard[11][0]-90<=countArr[11][0])&&(standard[11][0]+90>=countArr[11][0]))&&((standard[12][1]-50<=countArr[12][1])&&(standard[12][1]+50>=countArr[12][1]))
                    ||((standard[8][0]-90<=countArr[8][0])&&(standard[8][0]+90>=countArr[8][0]))&&((standard[9][1]-50<=countArr[9][1])&&(standard[9][1]+50>=countArr[9][1]))
            ){
                Log.i("drawBodyPoint호출됨",(++poseCount)+"");
                Camera2BasicFragment.setViewText(poseCount+"");
                Log.i("포즈 카운트ㅡ: ",poseCount+"");
                CountFlag.startTimer();
            }
    ...
    private void compareAccuracy(float[][] resultArr) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            percentageList.add("T:" + compareArr[i] + "  " +  "  A " + (i + 1)+": " + String.format("%.2f", betweenAngle) + "   Accur: " + String.format("%.2f", percentage) + "%");
        }
  ...
  */
    ...
    public void useGpu() {
        if (gpuDelegate == null && com.capstone.wowu.GpuDelegateHelper.isGpuDelegateAvailable()) {
            gpuDelegate = com.capstone.wowu.GpuDelegateHelper.createGpuDelegate();
